...
emb-c : {S : Stp} {Γ Δ : Cxt} {C : Fma} →
          S ∣ Γ ؛ Δ ⊢c C → S ∣ Γ ++ Δ ⊢ C
emb-c∙ : {S : Stp} {Γ Δ : TCxt} {Γ' Δ' : Cxt} {C : Fma} →
       ∙ ∣ S ∣ Γ ؛ Δ ⊢c C → (eq1 : Γ' ≡ ersL Γ) (eq2 : Δ' ≡ ersL Δ) → S ∣ Γ' ++ Δ' ⊢ C
...
emb-c∙ (ex∙ {Δ = Δ} f refl refl) refl refl = ex-cxt-fma (ersL Δ) (emb-c∙ f refl refl) -- ex-cxt-fma (ersL Δ) (emb-c∙ f)
emb-c∙ (ri2c f) refl refl = emb-ri∙ f refl -- emb-ri∙ f refl

emb-ri {Γ = Γ} (⊸r f) = ⊸r (ex-fma-cxt {Γ = []} {Δ = []} Γ (emb-c f))
emb-ri (li2ri f) = emb-li f

emb-ri∙ {Γ = Γ} (⊸r∙ f) refl = ⊸r (ex-fma-cxt {Γ = []} {Δ = []} (ersL Γ) (emb-c∙ f refl refl)) -- ⊸r (ex-fma-cxt {Γ = []} {Δ = []} (ersL Γ) (emb-c∙ f))
...
emb-p∙ (pass∙ f refl) = pass (emb-li f)
emb-p∙ (f2p f) = emb-f∙ f

emb-f ax = ax
emb-f Ir = Ir
emb-f {S} (⊗r {Γ = Γ} f g refl) = ⊗r (emb-ri∙ f (sym (tagErs Γ))) (emb-ri g)
emb-f (⊸l f g) = ⊸l (emb-ri f) (emb-li g)

emb-f∙ ax = ax
emb-f∙ Ir = Ir
emb-f∙ (⊗r∙ {Γ = Γ} {Δ = Δ} f g refl refl) = ⊗r (emb-ri∙ f (sym (whiteErs Γ))) (emb-ri g)
emb-f∙ (⊸l∙ f g refl refl refl) = ⊸l (emb-ri f) (emb-li g)
