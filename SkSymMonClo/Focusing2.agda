...
-- isInter++?[]right-refl : {X : Set} → (zs₀ zs₁ : List X) →
--                          isInter++? zs₀ zs₁ refl ([]right' (zs₀ ++ zs₁)) ≡ (zs₀ , zs₁ , [] , [] , refl , refl , []right' zs₀ , []right' zs₁)
--                         --  Σ (List X) (λ xs₀ → Σ (List X) (λ xs₁ → Σ (List X) (λ ys₀ → Σ (List X) (λ ys₁ → xs ≡ xs₀ ++ xs₁ × ys ≡ ys₀ ++ ys₁ × isInter xs₀ ys₀ zs₀ × isInter xs₁ ys₁ zs₁))))
-- isInter++?[]right-refl zs₀ zs₁ with isInter++? zs₀ zs₁ refl ([]right' (zs₀ ++ zs₁))
-- ... | .[] , .[] , [] , [] , refl , refl , isInter[] , isInter[] = refl
-- ... | .[] , .(_ ∷ _) , [] , [] , refl , refl , isInter[] , []right = refl
-- ... | .(_ ∷ _) , .[] , [] , [] , refl , refl , []right , isInter[] = refl
-- ... | .(_ ∷ _) , .(_ ∷ _) , [] , [] , refl , refl , []right , []right = refl

-- -- We need a ⊥ reasoning for isInter

-- -- isInter-refl-lem : {X : Set} → {x : X} → {xs ys zs : List X} → (eq1 : isInter (x ∷ xs) ys zs) → (eq2 : isInter xs (x ∷ ys) zs) → ∷right {x = x} {x} eq1 ≡ ∷left {x = x} {x} eq2
-- -- isInter-refl-lem []right []left = {!   !}
-- -- isInter-refl-lem []right (∷left eq2) = {!   !}
-- -- isInter-refl-lem []right (∷right eq2) = {!   !}
-- -- isInter-refl-lem (∷left eq1) eq2 = {!   !}
-- -- isInter-refl-lem (∷right eq1) eq2 = {!   !}

-- -- isInter-refl : {X : Set} → {xs ys zs : List X} → (eq1 : isInter xs ys zs) → (eq2 : isInter xs ys zs) → eq1 ≡ eq2
-- -- isInter-refl isInter[] isInter[] = refl
-- -- isInter-refl []left []left = {!   !}
-- -- isInter-refl []right []right = {!   !}
-- -- isInter-refl (∷left eq1) (∷left eq2) = {!   !}
-- -- isInter-refl (∷left eq1) (∷right eq2) = {!   !} -- why this case could occur?
-- -- isInter-refl (∷right {x = x} {y} eq1) (∷left {x = x₁} {y₁} eq2) = {!  !}
-- -- isInter-refl (∷right eq1) (∷right eq2) = {!   !}

[]++? : {X : Set} → {xs ys : List X} → [] ≡ xs ++ ys → xs ≡ [] × ys ≡ []
[]++? {xs = []} {[]} eq = refl , refl

-- list-refl-refl : {X : Set} → {x : X} → {xs ys zs : List X} → (eq1 : x ∷ xs ≡ ys ++ zs) → (eq2 : x ∷ xs ≡ ys ++ zs) → eq1 ≡ eq2
-- list-refl-refl {xs = xs} {ys} {zs} eq1 eq2 with cases++ [] ys xs zs (sym eq2)
-- list-refl-refl {xs = .(ds ++ zs)} {.(_ ∷ ds)} {zs} refl refl | inj₁ (ds , refl , refl) = refl
-- ... | inj₂ (ds , p , p') with []++? {xs = ys} {ds} p'
-- list-refl-refl {xs = xs} {.[]} {.([] ++ _ ∷ xs)} refl refl | inj₂ (.[] , refl , refl) | refl , refl = refl


-- isInter∷? : {X : Set} → {x y z : X} → {xs ys zs : List X} → isInter (x ∷ xs) (y ∷ ys) (z ∷ zs) → Σ (isInter xs (y ∷ ys) zs) (λ inTeq → x ≡ z) ⊎ Σ (isInter (x ∷ xs) ys zs) (λ inTeq → y ≡ z)
-- isInter∷? (∷left eq) = inj₁ (eq , refl)
-- isInter∷? (∷right eq) = inj₂ (eq , refl)

isInter++-∷left' : {X : Set} → {x : X} → (xs : List X) → {ys zs xs1 ys1 zs1 : List X} → (inTeq1 : isInter (x ∷ xs) ys (x ∷ zs)) → (inTeq' : isInter xs ys zs) → (inTeq2 : isInter xs1 ys1 zs1) → isInter++ inTeq1 inTeq2 ≡ ∷left' (ys ++ ys1) (isInter++ inTeq' inTeq2)
isInter++-∷left' .[] []right isInter[] inTeq2 = {!   !}
isInter++-∷left' .(_ ∷ _) []right []right inTeq2 = {!   !}
isInter++-∷left' xs (∷left inTeq1) inTeq' inTeq2 = {!   !}
isInter++-∷left' xs (∷right inTeq1) inTeq' inTeq2 = {!   !}
...
⊗rpass-ri {Γ₁' = []} Γ₁ (⊸r (ex {Δ = x ∷ Δ} {Λ} (ri2c f) refl refl)) g []right peq = ⊗rpass-ri (Γ₁ ++ _ ∷ []) f g  (∷left (isInter++l Δ (∷right' Λ ([]right' Λ)))) (snoc↭' {xs₀ = []} {[]} refl peq)
⊗rpass-ri {Γ₁' = x ∷ Γ₁'} Γ₁ (⊸r (ex {Δ = Δ} {Λ} (ri2c f) refl refl)) g eq peq with isInter++? Δ Λ refl eq
⊗rpass-ri {Γ₀ = _} {x ∷ Γ₁'} Γ₁ {A' = A'} (⊸r (ex {Δ = .[]} {Λ} (ri2c f) refl refl)) g eq peq | .[] , Γ₀₁ , [] , .(x ∷ Γ₁') , refl , refl , isInter[] , inTeq2 = ⊗rpass-ri (Γ₁ ++ _ ∷ []) f g (∷right' Γ₀₁ inTeq2) (snoc↭' {xs₀ = []} {xs₁ = x ∷ Γ₁'} {ys = Γ₁} refl peq)
⊗rpass-ri {Γ₀ = _} {x ∷ Γ₁'} Γ₁ {A' = A'} (⊸r (ex {Δ = .(_ ∷ _)} {Λ} (ri2c f) refl refl)) g eq peq | (_ ∷ xs) , Γ₀₁ , [] , .(x ∷ Γ₁') , refl , refl , []right , inTeq2 = ⊗rpass-ri (Γ₁ ++ _ ∷ []) f g (∷left (isInter++l xs (∷right' Γ₀₁ inTeq2))) (snoc↭' {xs₀ = []} {xs₁ = x ∷ Γ₁'} {ys = Γ₁} refl peq)
⊗rpass-ri {Γ₀ = _} {x ∷ .(Γ₁₀' ++ Γ₁₁')} Γ₁ {A' = A'} (⊸r (ex {Δ = Δ} {Λ} (ri2c f) refl refl)) g eq peq | Γ₀₀ , Γ₀₁ , .x ∷ Γ₁₀' , Γ₁₁' , refl , refl , inTeq1 , inTeq2 = ⊗rpass-ri (Γ₁ ++ _ ∷ []) f g (isInter++ inTeq1 (∷right' Γ₀₁ inTeq2)) (snoc↭' {xs₀ = x ∷ Γ₁₀'} {xs₁ = Γ₁₁'} {ys = Γ₁} refl peq)
...
⊗rpass-c-ri (ri2c (li2ri f)) g refl = refl
-- ⊗rpass-c-ri (ri2c (⊸r {Γ = Γ} {A = A} f)) g refl = {!   !}
-- rewrite []right-∷left' {x = A} {xs = Γ} = {!   !} 

-- cong (λ x → ex {Δ = []} (ri2c (li2ri x)) refl refl) (⊗rpass-ri {Γ = Γ} []  (⊸r {!   !}) g ([]right' Γ) (empty refl))
-- ⊗rpass-c-ri (ri2c (li2ri f)) g refl = {!   !}
...
-- ... | Γ₀₀ , Γ₀₁ , Γ₁₀' , Γ₁₁' , refl , refl , inTeq1 , inTeq2 = ⊗rIl-ri (Γ₁ ++ _ ∷ []) f g (isInter++ inTeq1 (∷right' Γ₀₁ inTeq2)) (snoc↭' refl peq)
...
isInter++-refl : {X : Set} → {xs₀ ys₀ zs₀ xs₁ ys₁ zs₁ : List X} → (inTeq1 : isInter xs₀ ys₀ zs₀) (inTeq2 : isInter xs₁ ys₁ zs₁) →  isInter++? zs₀ zs₁ refl (isInter++ inTeq1 inTeq2) ≡ (xs₀ , xs₁ , ys₀ , ys₁ , refl , refl , inTeq1 , inTeq2)

isInter++∷left' : {X : Set} → {xs₀ xs₁ ys zs₀ zs₁ : List X} → {x : X} → isInter (xs₀ ++ x ∷ xs₁) ys (zs₀ ++ x ∷ zs₁) →  isInter (xs₀ ++ xs₁) ys (zs₀ ++ zs₁)
isInter++∷left' {xs₀ = []} {zs₀ = []} inTeq = {!   !}
isInter++∷left' {xs₀ = []} {zs₀ = x ∷ zs₀} inTeq = {!   !}
isInter++∷left' {xs₀ = x ∷ xs₀} {zs₀ = []} inTeq = {!   !}
isInter++∷left' {xs₀ = x ∷ xs₀} {zs₀ = x₁ ∷ zs₀} inTeq = {!   !}
-- isInter++∷left' {xs₀ = []} {zs₀ = []} inTeq = ∷left' _ inTeq
-- isInter++∷left' {xs₀ = []} {zs₀ = x ∷ zs₀} inTeq = {!   !}
-- isInter++∷left' {xs₀ = x ∷ xs₀} {zs₀ = []} inTeq = {!   !}
-- isInter++∷left' {xs₀ = x ∷ xs₀} {zs₀ = x₁ ∷ zs₀} inTeq = {!   !}


⊗r⊗l-ri : {Γ Γ₀ Γ₀' Γ₀₀ Γ₀₀' Γ₁' Δ : Cxt} {A B C D : Fma}
        → (Γ₁ : Cxt) (f : just A ∣ Γ ⊢ri C) (g : - ∣ Δ ⊢ri D) (eq : Γ ≡ Γ₀ ++ B ∷ Γ₀') (inTeq : isInter (Γ₀₀ ++ Γ₀₀') Γ₁' (Γ₀ ++ Γ₀')) (peq : Γ₁' ↭' Γ₁) 
        → gen⊗r-ri Γ₁ (⊗l-ri f eq) g inTeq peq ≡ ⊗l (ex (ri2c (li2ri (gen⊗r-ri Γ₁ f g (subst (isInter (Γ₀₀ ++ B ∷ Γ₀₀') Γ₁') (sym eq) {!   !}) peq))) refl refl)
...
--         → gen⊗r-ri Γ₁ (⊗l-ri f eq) g (isInter++ inTeq1 inTeq2) peq ≡ ⊗l (ex (ri2c (li2ri (gen⊗r-ri Γ₁ f g (subst (isInter (Γ₀₀ ++ B ∷ Γ₀₀') (Γ₁₀' ++ Γ₁₁')) (sym eq) ((isInter++ inTeq1 (∷left' Γ₁₁' inTeq2) ))) peq))) refl refl)

⊗r⊗l-ri Γ₁ (⊸r (ex (ex f refl refl) eq' eq₁)) g eq inTeq1 peq = {!   !}
⊗r⊗l-ri {Γ₀ = Γ₀} {Γ₀'} Γ₁ (⊸r (ex {Δ = Δ} {Λ} (ri2c f) refl refl)) g eq inTeq1 peq with cases++ Γ₀ Δ Γ₀' Λ eq 
⊗r⊗l-ri {Γ₀ = Γ₀} {.(Ω₀ ++ Λ)} Γ₁ (⊸r (ex {Δ = .(Γ₀ ++ _ ∷ Ω₀)} {Λ} (ri2c f) refl refl)) g refl inTeq1 peq | inj₁ (Ω₀ , refl , refl) = {!   !} -- rewrite isInter++-refl inTeq1 inTeq2 = {!   !}
⊗r⊗l-ri {Γ₀ = .(Δ ++ Ω₀)} {Γ₀'} Γ₁ (⊸r (ex {Δ = Δ} {.(Ω₀ ++ _ ∷ Γ₀')} (ri2c f) refl refl)) g refl inTeq1 peq | inj₂ (Ω₀ , refl , refl) = {!   !}
⊗r⊗l-ri Γ₁ (li2ri f) g eq inTeq1 peq = {!   !}

⊗r⊗l-c-ri : {Γ Γ' Ω Δ : Cxt} {A B C D : Fma}
          → (f : just A ∣ Γ' ؛ Ω ⊢c C) (g : - ∣ Δ ⊢ri D) (eq : Γ' ≡ B ∷ Γ)
          → ⊗r-c-ri (⊗l-c' f eq) g ≡ ⊗l-c' (⊗r-c-ri f g) eq

⊗r⊗l-c-ri (ex {Γ = []} (ex f eq' eq₁) eq eq1) g eq2 = {!   !}
⊗r⊗l-c-ri (ex {Γ = []} {Δ = Δ} {Λ} (ri2c f) refl refl) g refl = cong (λ x → ri2c (li2ri x)) {! ⊗r⊗l-ri [] f g refl ([]right' (Δ ++ Λ)) (empty refl)  !} --  (⊗r⊗l-ri [] f g refl {! []right' Δ  !} {!   !} (empty refl))
-- (⊸r (ex (ex f refl refl) eq' eq₁))) eq eq1) g eq2 = {!   !}
-- ⊗r⊗l-c-ri (ex {Γ = []} {Δ = Δ₁} {Λ = Λ₁} (ri2c (⊸r (ex {Δ = Δ} {Λ} (ri2c f) refl refl))) refl eq1) g refl with cases++ Δ₁ Δ Λ₁ Λ eq1
-- ... | inj₁ (Ω₀ , p , p') = {!   !}
-- ⊗r⊗l-c-ri (ex {_} {[]} {Δ = .(Δ ++ Ω₀)} {Λ = Λ₁} (ri2c (⊸r (ex {Δ = Δ} {.(Ω₀ ++ _ ∷ Λ₁)} (ri2c f) refl refl))) refl refl) g refl | inj₂ (Ω₀ , refl , refl) = {!   !}
-- ⊗r⊗l-c-ri (ex {Γ = []} {[]} {[]} (ri2c (li2ri f)) refl refl) g refl = refl
-- ⊗r⊗l-c-ri (ex {Γ = []} {[]} {x ∷ Λ} (ri2c (li2ri f)) refl refl) g refl = refl
-- ⊗r⊗l-c-ri (ex {Γ = []} {x ∷ Δ} {[]} (ri2c (li2ri f)) refl refl) g refl = refl
-- ⊗r⊗l-c-ri (ex {Γ = []} {x ∷ Δ} {x₁ ∷ Λ} (ri2c (li2ri f)) refl refl) g refl = refl
⊗r⊗l-c-ri (ex {Γ = x ∷ Γ} f refl refl) g refl = {!   !}


⊗r⊗l-c : {Γ Γ' Δ Λ : Cxt} {A B C D : Fma}
       → (f : just A ∣  Γ' ؛ [] ⊢c C) (g : - ∣ Δ ؛ Λ ⊢c D) (eq : Γ' ≡ B ∷ Γ)
       → ⊗r-c (⊗l-c' f eq) g ≡ ⊗l-c' (⊗r-c f g) (cong (_++ Δ) eq)
⊗r⊗l-c f (ex g refl refl) refl = {!   !}
⊗r⊗l-c f (ri2c g) refl = {!   !}
...
⊗r⊸l-ri Γ₁ f (⊸r (ex (ex g refl refl) eq' eq)) h inTeq peq = {!   !} -- imposs
⊗r⊸l-ri Γ₁ f ((⊸r (ex {Δ = Δ} {Λ} (ri2c g) refl refl))) h inTeq peq = {!   !}
-- ⊗r⊸l-ri {[]} {Γ₁' = []} Γ₁ f (⊸r (ex {Δ = []} {.[]} (ri2c g) refl refl)) h isInter[] peq = ⊗r⊸l-ri (Γ₁ ++ _ ∷ []) f g h []left (snoc↭' {xs₀ = []} {[]} refl peq)
-- ⊗r⊸l-ri {x ∷ Δ} {Γ₁' = []} Γ₁ f (⊸r (ex {Δ = []} {.[]} (ri2c g) refl refl)) h isInter[] peq = ⊗r⊸l-ri (Γ₁ ++ _ ∷ []) f g h []left (snoc↭' {xs₀ = []} {[]} refl peq)
-- ⊗r⊸l-ri {[]} {Γ₁' = []} Γ₁ f (⊸r (ex {Δ = []} {.(_ ∷ _)} (ri2c g) refl refl)) h []right peq = ⊗r⊸l-ri (Γ₁ ++ _ ∷ []) f g h (∷right []right) (snoc↭' {xs₀ = []} {[]} refl peq)
-- ⊗r⊸l-ri {x ∷ Δ} {Γ₁' = []} Γ₁ f (⊸r (ex {Δ = []} {.(_ ∷ _)} (ri2c g) refl refl)) h []right peq = ⊗r⊸l-ri (Γ₁ ++ _ ∷ []) f g h (∷right []right) (snoc↭' {xs₀ = []} {[]} refl peq)
-- ⊗r⊸l-ri {Δ = []} {Γ₁' = []} Γ₁ f (⊸r (ex {Δ = x ∷ Δ} {Λ} (ri2c g) refl refl)) h []right peq = ⊗r⊸l-ri (Γ₁ ++ _ ∷ []) f g h (∷left (isInter++l Δ (∷right' Λ ([]right' Λ)))) (snoc↭' {xs₀ = []} {[]} refl peq)
-- ⊗r⊸l-ri {Δ = x₁ ∷ Δ₁} {Γ₁' = []} Γ₁ f (⊸r (ex {Δ = x ∷ Δ} {Λ} (ri2c g) refl refl)) h []right peq = {!  ⊗r⊸l-ri (Γ₁ ++ _ ∷ []) f g h (∷left (isInter++l Δ (∷right' Λ ([]right' Λ)))) (snoc↭' {xs₀ = []} {[]} refl peq) !}
-- ⊗r⊸l-ri {Δ = []} {Γ₁' = x ∷ Γ₁'} Γ₁ f (⊸r (ex {Δ = Δ} {Λ} (ri2c g) refl refl)) h inTeq peq = {!   !}
-- ⊗r⊸l-ri {Δ = x₁ ∷ Δ₁} {Γ₀ = []} {Γ₁' = x ∷ Γ₁'} Γ₁ f (⊸r (ex {Δ = []} {.x ∷ .Γ₁'} (ri2c g) refl refl)) h []left peq = {!   !}
-- ⊗r⊸l-ri {Δ = x₁ ∷ Δ₁} {Γ₀ = []} {Γ₁' = x ∷ Γ₁'} Γ₁ f (⊸r (ex {Δ = x₂ ∷ Δ} {Λ} (ri2c g) refl refl)) h inTeq peq = {!   !}
-- ⊗r⊸l-ri {Δ = x₁ ∷ Δ₁} {Γ₀ = x₂ ∷ Γ₀} {Γ₁' = x ∷ Γ₁'} Γ₁ f (⊸r (ex {Δ = Δ} {Λ} (ri2c g) refl refl)) h inTeq peq = {!   !}


-- with isInter++? Δ Λ refl inTeq
-- ... | .[] , Γ₀₁ , [] , .(x ∷ Γ₁') , refl , refl , isInter[] , inTeq2 = {!   !}
-- ... | .(_ ∷ _) , Γ₀₁ , [] , .(x ∷ Γ₁') , refl , refl , []right , inTeq2 = {!   !}
-- ... | Γ₀₀ , Γ₀₁ , .x ∷ Γ₁₀' , Γ₁₁' , refl , refl , inTeq1 , inTeq2 = {!   !}
⊗r⊸l-ri Γ₁ f (li2ri g) h inTeq peq = {!   !}

⊗r⊸l-c-ri : {Γ Δ Λ Ω Θ : Cxt} {A B C D :  Fma}
         → (f : - ∣ Γ ؛ Δ ⊢c A) (g : just B ∣ Λ ⊢ri C) (h : - ∣ Ω ⊢ri D)
         → ⊗r-c-ri (⊸l-c-ri f g) h ≡ ⊸l-c-ri f (li2ri (gen⊗r-ri [] g h ([]right' Λ) (empty refl)))

⊗r⊸l-c-ri (ex f refl refl) g h = {!   !}
